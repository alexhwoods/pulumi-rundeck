...
First, clone this repo with the name of the desired provider in place of `rundeck`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-rundeck
```

Second, replace references to `rundeck` with the name of your provider:
...
To use from Go, use `go get` to grab the latest version of the library
    ...
    $ go get github.com/pulumi/pulumi-rundeck/sdk/go/...

## Configuration

The following configuration points are available for the `rundeck` provider:

- `rundeck:apiKey` (environment: `XYZ_API_KEY`) - the API key for `rundeck`
- `rundeck:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
